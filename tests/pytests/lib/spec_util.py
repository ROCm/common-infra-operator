...
def generate_k8_deviceconfig_cr(gpu_operator_version, spec = {}, skip_sections = {}):
    ...
    if not skip_sections.get('commonConfig', False):
        ...
        if spec.get('remediationWorkflow.configMapImage.secret', None):
            secrets = device_config['spec']['commonConfig'].setdefault('imageRegistrySecrets', [])
            secret_name = spec.get('remediationWorkflow.configMapImage.secret')
            if {'name': secret_name} not in secrets:
    ...
    if not skip_sections.get('driver', False):
        ...
        if gpu_op_version >= version.Version('v1.2.0'):
            ...
            if spec.get('driver.imageBuild.sourceImageRepo', None):
                device_config['spec']['driver']['imageBuild']['sourceImageRepo'] = spec['driver.imageBuild.sourceImageRepo']
        if spec.get('driver.useSourceImage', False):
            device_config['spec']['driver']['useSourceImage'] = True
        if spec.get('driver.imageRegistrySecret', None):
            device_config['spec']['driver']['imageRegistrySecret'] = {
                'name': spec['driver.imageRegistrySecret']
            }
    else:
    ...
    if not skip_sections.get('devicePlugin', False):
        ...
        dp_secret = spec.get('devicePlugin.devicePluginImage.secret') or spec.get('devicePlugin.nodeLabellerImage.secret')
        if dp_secret:
    ...
    if 'draDriver' in device_config['spec']:
        if not skip_sections.get('draDriver', False):
            ...
            cmd_line_args = spec.get('draDriver.cmdLineArguments', {})
            if cmd_line_args:
                device_config['spec']['draDriver']['cmdLineArguments'] = cmd_line_args
            else:
                device_config['spec']['draDriver'].pop('cmdLineArguments', None)
            tolerations = spec.get('draDriver.tolerations', [])
            if tolerations:
                device_config['spec']['draDriver']['tolerations'] = tolerations
            else:
                device_config['spec']['draDriver'].pop('tolerations', None)
            dra_selector = spec.get('draDriver.selector', {})
            if dra_selector:
                device_config['spec']['draDriver']['selector'] = dra_selector
            else:
                device_config['spec']['draDriver'].pop('selector', None)
        else:
    ...
    if 'remediationWorkflow' in device_config['spec']:
        if not skip_sections.get('remediationWorkflow', False):
            ...
            if spec.get('remediationWorkflow.ttlForFailedWorkflows', None):
                device_config['spec']['remediationWorkflow']['ttlForFailedWorkflows'] = spec.get('remediationWorkflow.ttlForFailedWorkflows')
            if spec.get('remediationWorkflow.config', None):
                device_config['spec']['remediationWorkflow']['config'] = {
                    'name' : spec.get('remediationWorkflow.config'),
                }
            ...
            if spec.get('remediationWorkflow.configMapImage.repository', None) and spec.get('remediationWorkflow.configMapImage.version', None):
                img = f"{spec.get('remediationWorkflow.configMapImage.repository')}:{spec.get('remediationWorkflow.configMapImage.version')}"
                device_config['spec']['remediationWorkflow']['configMapImage'] = img
            if spec.get('remediationWorkflow.nodeRemediationTaints', None) is not None:
                device_config['spec']['remediationWorkflow']['nodeRemediationTaints'] = spec.get('remediationWorkflow.nodeRemediationTaints')
            if spec.get('remediationWorkflow.nodeDrainPolicy.force', None) is not None:
                device_config['spec']['remediationWorkflow'].setdefault('nodeDrainPolicy', {})['force'] = spec.get('remediationWorkflow.nodeDrainPolicy.force')
            if spec.get('remediationWorkflow.nodeDrainPolicy.timeoutSeconds', None) is not None:
                device_config['spec']['remediationWorkflow'].setdefault('nodeDrainPolicy', {})['timeoutSeconds'] = spec.get('remediationWorkflow.nodeDrainPolicy.timeoutSeconds')
            if spec.get('remediationWorkflow.nodeDrainPolicy.gracePeriodSeconds', None) is not None:
                device_config['spec']['remediationWorkflow'].setdefault('nodeDrainPolicy', {})['gracePeriodSeconds'] = spec.get('remediationWorkflow.nodeDrainPolicy.gracePeriodSeconds')
            if spec.get('remediationWorkflow.nodeDrainPolicy.ignoreDaemonSets', None) is not None:
                device_config['spec']['remediationWorkflow'].setdefault('nodeDrainPolicy', {})['ignoreDaemonSets'] = spec.get('remediationWorkflow.nodeDrainPolicy.ignoreDaemonSets')
            if spec.get('remediationWorkflow.nodeDrainPolicy.ignoreNamespaces', None) is not None:
                device_config['spec']['remediationWorkflow'].setdefault('nodeDrainPolicy', {})['ignoreNamespaces'] = spec.get('remediationWorkflow.nodeDrainPolicy.ignoreNamespaces')
        else:
...
def generate_helmchart_deployment_config(gpu_operator_version, images, secret_list, file_name):
    ...
    # gpu-operator-utils container (used by pre-delete hook job)
    utils_container_prefix = 'commonConfig.utilsContainer'
    if images.get(f'{utils_container_prefix}.repository', None):
        modifed = True
        img = images.get(f'{utils_container_prefix}.repository') + ":" + images.get(f'{utils_container_prefix}.version', 'latest')
        if 'commonConfig' not in helmchart_values:
            helmchart_values['commonConfig'] = {}
        if 'utilsContainer' not in helmchart_values['commonConfig']:
            helmchart_values['commonConfig']['utilsContainer'] = {}
        helmchart_values['commonConfig']['utilsContainer']['image'] = img
    if images.get(f'{utils_container_prefix}.secret', None):
        modifed = True
        if 'commonConfig' not in helmchart_values:
            helmchart_values['commonConfig'] = {}
        if 'utilsContainer' not in helmchart_values['commonConfig']:
            helmchart_values['commonConfig']['utilsContainer'] = {}
        helmchart_values['commonConfig']['utilsContainer']['imageRegistrySecret'] = {
            'name': images.get(f'{utils_container_prefix}.secret')
        }
