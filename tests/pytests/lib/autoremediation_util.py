...
def install_argo_workflows_helm(namespace: str = "argo-workflow",
    ...
    """
    try:
        # Step 1: Install CRDs separately via server-side apply before Helm.
        # Per docs: CRDs must be installed with --server-side --force-conflicts due to size
        # constraints, then Helm is invoked with crds.install=false.
        if install_crds:
            Logger.info(f"Installing Argo CRDs separately (git tag: {argo_git_tag})")
            ret_code, stdout, stderr = install_argo_crds(version=argo_git_tag)
            if ret_code != 0:
                return -1, stdout, f"Failed to install Argo CRDs: {stderr}"
        ...
        # CRDs are installed separately (step 1), so crds.install=false here.
        ...
        helm_install_cmd = [
            ...
            # Tolerate amd-gpu-unhealthy:NoSchedule so the controller can reschedule on the node
            # after it is tainted during remediation (node reboot brings the taint back up).
...
def get_anr_workflow_sa(namespace: str) -> Optional[str]:
    """Return the SA name used by ANR workflows in the given namespace.

    The operator selects the first SA whose name ends with 'controller-manager'
    (mirrors remediationMgrHelper.getServiceAccountName in remediation_handler.go).
    Returns None if no matching SA is found.
    """
    v1 = client.CoreV1Api()
    try:
        for sa in v1.list_namespaced_service_account(namespace).items:
            if sa.metadata.name.endswith("controller-manager"):
                return sa.metadata.name
    except Exception as exc:
        Logger.warning(f"Could not list service accounts in {namespace}: {exc}")
    return None
