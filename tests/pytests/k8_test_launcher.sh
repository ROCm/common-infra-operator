...
function usage() {
    ...
    echo "          --app <app-name>. Eq: gpu-operator, gpu-operator-exporter, gpu-operator-amdgpu-driver"
    ...
    echo "          --deployment <mode>. Eq: k8, openshift, standalone, hypervisor (default: k8)"
    echo "          --testbed <path-to-testbed-json>"
    echo "          --nic-config <path-to-nic-config-json>"
...
function launch_pytest() {
    ...
    if [[ "${TESTBED_JSON}" != "NA" ]];
    then
        CMD_OPT+=" --testbed ${TESTBED_JSON}"
    fi
    if [[ "${NIC_CONFIG}" != "NA" ]];
    then
        CMD_OPT+=" --nic-config ${NIC_CONFIG}"
    fi
    ...
    # Operator-specific test dirs (gpu-operator-*, network-operator-*) live outside
    # ${DEPLOYMENT}/${APP_NAME}/ so they are never collected during sanity runs.
    # To run them, use --app <dir-name> (e.g. --app gpu-operator-amdgpu-driver).
    pytest ${test_sel} \
