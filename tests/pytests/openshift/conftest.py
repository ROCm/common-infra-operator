...
def gpu_operator_install(gpu_cluster, gpu_operator_release_name, images, environment):
    ...
    # Force-clean all OLM artifacts before fresh install (handles partial/broken prior installs)
    olm_util.olm_force_cleanup(gpu_cluster, gpu_operator_release_name, environment.gpu_operator_namespace)

    if images.get('gpu-operator.olm-subscription'):
        # OLM Subscription path — install from certified-operators catalog
        ret_code, ret_stdout, ret_stderr = olm_util.olm_subscription_install(
            namespace=environment.gpu_operator_namespace,
            catalog=images['gpu-operator.olm-subscription.catalog'],
            channel=images['gpu-operator.olm-subscription.channel'],
            package=images['gpu-operator.olm-subscription.package'],
            csv=images['gpu-operator.olm-subscription.csv'],
        )
        K8Helper.triage(environment, ret_code == 0,
                        f"Failed to install via OLM Subscription: {ret_stderr}")
    else:
        # operator-sdk run bundle path (nightly builds)
        opts = {}
        ...
        # Approve any Manual-approval install plans (operator-sdk sets approval=Manual by default)
        olm_util.olm_approve_install_plans(environment.gpu_operator_namespace)
        time.sleep(15)

    # Verify CRD is registered and CSV reached Succeeded before running any tests
    healthy = olm_util.olm_verify_healthy(environment.gpu_operator_namespace, gpu_operator_release_name)
    K8Helper.triage(environment, healthy,
                    "OLM install unhealthy: deviceconfigs.amd.com CRD not registered or CSV not Succeeded")
...
def argo_workflow_setup(gpu_cluster, environment, request):
    ...
    # Chart v1.0.6 includes Argo Workflows v4.0.3
    chart_version = "1.0.6"
    ...
    # Step 4: Bind the privileged SCC to the ANR workflow service account.
    ...
    if workflow_sa:
        Logger.info(f"Binding privileged SCC to ANR workflow SA '{workflow_sa}' in '{gpu_operator_namespace}'")
        ret_code, _, ret_stderr = k8_util.k8_create_role_binding(
            _scc_crb_name,
        ...
        K8Helper.triage(environment, (ret_code == 0),
                        f"Failed to bind privileged SCC to '{workflow_sa}': {ret_stderr}")
        argo_info["scc_crb_name"] = _scc_crb_name
    else:
        K8Helper.triage(environment, False,
                        f"No controller-manager SA found in '{gpu_operator_namespace}'; "
                        "cannot bind privileged SCC — check that the GPU operator is installed "
                        "and running in the correct namespace")

    # Register cleanup finalizer BEFORE yield (during setup phase)
    def _cleanup_argo():
        if argo_info.get("scc_crb_name"):
            Logger.info(f"Removing privileged SCC ClusterRoleBinding '{argo_info['scc_crb_name']}'")
            k8_util.k8_delete_cluster_role_binding(argo_info["scc_crb_name"])
