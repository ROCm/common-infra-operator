...
"""
VF Metrics Accuracy Test Suite — Docker Container Deployment.
...
The exporter container is started by run_sriov_exporter_docker (module-scoped,
from hypervisor/docker/conftest.py) and is shared with test_vf_metrics.py
when tests are collected from the same directory.
"""
...
Logger = logging.getLogger("hypervisor.docker.test_metrics_accuracy")
...
def _prom_key(json_name):
    """Convert a metrics-support.json name to its Prometheus dict key (amd_ prefix)."""
    base = json_name.split(':')[0].lower()
    return f'amd_{base}'
...
def metrics_samples(gim_node, hypervisor_node, vf_topology,
                    run_sriov_exporter_docker, environment):
    """
