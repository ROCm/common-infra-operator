...
"""
VF Activity Metrics Test Suite — Docker Container Deployment.
...
The exporter is deployed by the module-scoped run_sriov_exporter_docker
fixture imported from the hypervisor/docker test suite.  The vf_topology
...
Logger = logging.getLogger("hypervisor.docker.test_vf_metrics")
...
def vm0_workload(vf_topology, gim_node, hypervisor_node, run_sriov_exporter_docker,
                 environment):
    """
    ...
    # only exists after a successful probe).  Hard fail — not skip — so CI
    # catches a missing driver rather than silently skipping all workload tests.
...
def test_vf_gfx_activity_under_workload(
    gim_node, hypervisor_node, vf_topology, run_sriov_exporter_docker,
