...
"""
test_update_1.1.0_operand.py

Updates NetworkConfig operand images to v1.1.0 release images from the
docker.io/rocm registry.

  - spec.devicePlugin.devicePluginImage  -> docker.io/rocm/k8s-network-device-plugin:v1.1.0
  - spec.devicePlugin.nodeLabellerImage  -> docker.io/rocm/k8s-network-node-labeller:v1.1.0
  - spec.metricsExporter.image           -> docker.io/rocm/device-metrics-exporter:nic-v1.1.0
  - spec.cniPlugins.image               -> docker.io/rocm/k8s-cni-plugins:v1.1.0
"""
...
# Fixed v1.1.0 release images
DEVICE_PLUGIN_IMAGE    = "docker.io/rocm/k8s-network-device-plugin:v1.1.0"
NODE_LABELLER_IMAGE    = "docker.io/rocm/k8s-network-node-labeller:v1.1.0"
METRICS_EXPORTER_IMAGE = "docker.io/rocm/device-metrics-exporter:nic-v1.1.0"
CNI_PLUGINS_IMAGE      = "docker.io/rocm/k8s-cni-plugins:v1.1.0"
...
def _assert_pods_have_image(
    ...
    while time.monotonic() < deadline:
        ...
        for pod in pods:
            ...
            if actual_expected not in running_images:
                failures.append(
...
def test_update_1_1_0_device_plugin_image():
    """Update spec.devicePlugin.devicePluginImage to v1.1.0."""
...
@pytest.mark.timeout(TEST_TIMEOUT)
def test_update_1_1_0_node_labeller_image():
    """Update spec.devicePlugin.nodeLabellerImage to v1.1.0."""
...
@pytest.mark.timeout(TEST_TIMEOUT)
def test_update_1_1_0_metrics_exporter_image():
    """Update spec.metricsExporter.image to nic-v1.1.0."""
...
@pytest.mark.timeout(TEST_TIMEOUT)
def test_update_1_1_0_cni_plugin_image():
    """Update spec.cniPlugins.image to v1.1.0."""
...
@pytest.mark.timeout(TEST_TIMEOUT)
def test_update_1_1_0_all_operand_images():
    """
    Update all four operand images to v1.1.0 in a single pass per NetworkConfig.
    """
    LOG.info("Target images (v1.1.0):")
    ...
    for nc in nc_items:
        ...
        if not (dp_needs or nl_needs or me_needs):
            LOG.info("[%s] All operand images are already at v1.1.0", name)
    ...
    if not updated:
        pytest.skip("All operand images are already at v1.1.0")
