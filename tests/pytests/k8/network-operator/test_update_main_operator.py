...
"""
test_update_main_operator.py

Upgrades the amd-network-operator Helm release to the latest main-* build
...
NETWORK_OPERATOR_HELM_TARBALL_PATTERN = (
    "amdpsdo-network-operator-helm-k8s-v0.0.1-{tag}.tgz"
)
...
def _helm_current_operator_tag(master: dict) -> str:
    ...
    e.g. ``main-95``, by parsing ``helm list -A`` output.
    """
    out = _run_on_master(master, "helm list -A --no-headers", timeout=30)

    for line in out.splitlines():
        if NETWORK_OPERATOR_HELM_RELEASE in line:
            # columns: NAME  NAMESPACE  REVISION  UPDATED  STATUS  CHART  APP_VERSION
            parts = line.split()
            if parts:
                return parts[-1].strip()   # APP VERSION is the last column
    raise RuntimeError(
...
def test_update_operator():
    """
    Upgrade the amd-network-operator Helm release to the latest main-* build
    published on the assets server.

    Steps:
      1. Detect the currently deployed APP VERSION via ``helm list -A``.
      2. Scrape the assets server for the highest ``main-<N>`` build.
      3. Skip if the release is already at the latest build.
      4. Download the helm chart tarball for the latest build.
      5. Run ``helm upgrade`` with the required imagePullSecrets.
      6. Confirm the release APP VERSION matches the new tag.
    """
    if shutil.which("sshpass") is None or shutil.which("ssh") is None:
    ...
    latest_tag = _latest_main_tag(NETWORK_OPERATOR_ASSETS_URL)
    ...
    # Build the download URL for the tarball
    ...
    # Confirm the running release now reports the new tag
    try:
