...
"""
test_update_1.1.0_operator.py

Upgrades the amd-network-operator Helm release to v1.1.0 using the
...
def _helm_upgrade_operator_on_master(master: dict) -> None:
    """Run helm upgrade to v1.1.0 using the rocm-network helm repo."""
    cmd = (
...
def test_update_1_1_0_operator():
    """
    Upgrade the amd-network-operator Helm release to v1.1.0 using
    rocm-network/network-operator-charts. Skips if already at v1.1.0.
    """
    if shutil.which("sshpass") is None or shutil.which("ssh") is None:
