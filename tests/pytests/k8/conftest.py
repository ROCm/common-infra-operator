    ...
    def _cleanup_steps():
        ...
        if hasattr(environment, "gpu_operator_namespace"):
            ...
            if helm_util.is_helm_release_present(gpu_cluster, gpu_operator_release_name, environment.gpu_operator_namespace):
                ...
                helm_util.helm_uninstall_with_recovery(gpu_cluster, gpu_operator_release_name,
                                                       environment.gpu_operator_namespace)

        if hasattr(environment, "exporter_namespace"):
            if helm_util.is_helm_release_present(gpu_cluster, exporter_release_name, environment.exporter_namespace):
...
def gpu_operator_install(gpu_cluster, gpu_operator_release_name, images, environment):
    ...
    ret_code, ret_stdout, ret_stderr = helm_util.helm_uninstall_with_recovery(gpu_cluster, gpu_operator_release_name, environment.gpu_operator_namespace)
