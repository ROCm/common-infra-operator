...
"""
conftest.py — pytest configuration for Device Plugin standalone Helm chart tests.
...
def pytest_configure(config):
    ...
    config._dp_log_file = log_file
    config._dp_log_handler = fh
    logging.getLogger("test_device_plugin_standalone_helm").info(
        "Log file: %s", log_file
    )
...
def pytest_unconfigure(config):
    ...
    log_handler = getattr(config, "_dp_log_handler", None)
    if log_handler is not None:
        root_logger = logging.getLogger()
        root_logger.removeHandler(log_handler)
        log_handler.close()
    log_file = getattr(config, "_dp_log_file", None)
