...
def test_deviceconfig_dra_driver_cmd_line_arguments(
    ...
    for pod in dra_pods:
        pod_name = pod["metadata"]["name"]
        Logger.info(f"Fetching logs from pod {pod_name} (since=120s)")
        ret_code, logs, err = k8_util.k8_get_pod_logs(
            pod_name, environment.gpu_operator_namespace, since="120s"
        )
        if ret_code == 0 and logs:
            ...
            if total_lines == 0:
                Logger.warning(f"No log lines fetched from pod {pod_name} — cannot validate log format")
            else:
                ...
                if non_json_lines:
                    Logger.info(f"Non-JSON lines ({len(non_json_lines)}): {non_json_lines[:5]}")
                debug_on_failure(
                    environment,
                    (json_ratio > 0.5),
                    f"Expected JSON-formatted logs with logging-format=json, "
                    f"but only {json_ratio:.0%} of {total_lines} lines are valid JSON. "
                    f"Non-JSON samples: {non_json_lines[:3]}",
                )
        else:
            Logger.warning(f"Could not collect logs from DRA driver pod {pod_name}: {err}")
