...
"""
conftest.py — pytest configuration for Node Labeller standalone Helm chart tests.
...
def pytest_configure(config):
    ...
    config._nl_log_file = log_file
    config._nl_log_handler = fh
    logging.getLogger("test_node_labeller_standalone_helm").info(
        "Log file: %s", log_file
    )
...
def pytest_unconfigure(config):
    ...
    log_handler = getattr(config, "_nl_log_handler", None)
    if log_handler is not None:
        root_logger = logging.getLogger()
        root_logger.removeHandler(log_handler)
        log_handler.close()
    log_file = getattr(config, "_nl_log_file", None)
